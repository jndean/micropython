...
int main() {
  ...
  VertexRef int_vtx = graph.addVertex(init_computeset, "InitVertex", {{"printBuf", printBuf}});
  VertexRef body_vtx = graph.addVertex(body_computeset, "RTVertex", {{"printBuf", printBuf}});
  graph.setTileMapping(int_vtx, 0);
  graph.setTileMapping(body_vtx, 0);
  
  // Create and run program 
  poplar::program::Sequence program({
    poplar::program::Execute(init_computeset),
    poplar::program::Execute(body_computeset)
  });
